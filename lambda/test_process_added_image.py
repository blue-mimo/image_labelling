...
class TestLambdaFunction(unittest.TestCase):
    ...
    def test_lambda_handler_success(
        ...
        mock_rekognition.detect_labels.assert_called_once_with(
            Image={
                "S3Object": {"Bucket": "test-bucket", "Name": "uploads/test-image.jpg"}
            },
            MaxLabels=10,
            MinConfidence=75,
        )

        # Verify DynamoDB put_item was called (3 label records)
        ...
        self.assertEqual(item["confidence"], Decimal("98.50"))

    @patch("process_added_image.get_dynamodb_resource")
    @patch("process_added_image.get_rekognition_client")
    @patch("process_added_image.get_s3_client")
    def test_lambda_handler_with_url_encoded_key(
        self, mock_get_s3, mock_get_rekognition, mock_get_dynamodb
    ):
    ...
    def test_rekognition_error(
        ...
        mock_rekognition.detect_labels.side_effect = Exception(
    ...
    @patch("process_added_image.get_rekognition_client")
    @patch("process_added_image.get_s3_client")
    def test_empty_labels_response(
        self, mock_get_s3, mock_get_rekognition, mock_get_dynamodb
    ):
        ...
        # Verify no labels were saved to DynamoDB (no records for empty labels)
        mock_table.put_item.assert_not_called()
    ...
    def test_different_file_extensions(
        ...
        for filename in test_files:
            with self.subTest(filename=filename):
                ...
                # Verify correct image name stored in DynamoDB (1 label record)
    ...
    def test_decimal_precision(
        ...
        # Should have 1 call: 1 label record
        self.assertEqual(mock_table.put_item.call_count, 1)
        ...
        self.assertEqual(call_args["Item"]["confidence"], Decimal("99.123456789"))

    @patch("process_added_image.get_dynamodb_resource")
    @patch("process_added_image.get_rekognition_client")
    @patch("process_added_image.get_s3_client")
    def test_empty_event_records(
        self, mock_get_s3, mock_get_rekognition, mock_get_dynamodb
    ):
